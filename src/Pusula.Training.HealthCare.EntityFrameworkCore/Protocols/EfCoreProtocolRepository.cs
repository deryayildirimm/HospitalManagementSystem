    ...
    : EfCoreRepository<HealthCareDbContext, Protocol, Guid>(dbContextProvider), IProtocolRepository
{
    ...
    protected virtual async Task<IQueryable<ProtocolWithNavigationProperties>> GetQueryForNavigationPropertiesAsync()
    {
        return from protocol in (await GetDbSetAsync())
            join patient in (await GetDbContextAsync()).Set<Patient>() on protocol.PatientId equals patient.Id into
                patients
            from patient in patients.DefaultIfEmpty()
            join department in (await GetDbContextAsync()).Set<Department>() on protocol.DepartmentId equals
                department.Id into departments
        ...
        Guid? departmentId = null)
    {
        return query
            .WhereIf(!string.IsNullOrWhiteSpace(filterText),
                e => e.Protocol.Type!.Contains(filterText!) || e.Protocol.EndTime!.Contains(filterText!))
            ...
            .WhereIf(!string.IsNullOrWhiteSpace(endTime),
                e => e.Protocol.EndTime != null && e.Protocol.EndTime.Contains(endTime!))
            .WhereIf(patientId != null && patientId != Guid.Empty, e => e.Patient != null && e.Patient.Id == patientId)
            .WhereIf(departmentId != null && departmentId != Guid.Empty,
        ...
        string? endTime = null)
    {
        return query
            .WhereIf(!string.IsNullOrWhiteSpace(filterText),
                e => e.Type!.Contains(filterText!) || e.EndTime!.Contains(filterText!))
