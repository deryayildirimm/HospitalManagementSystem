...
public class IcdAdaptor (IIcdsAppService icdAppService) : DataAdaptor
{
        ...
        string? additionalParam = null)
    {
        try
        {
            ...
            var filter = string.IsNullOrEmpty(filterJson)
                ? new GetIcdsInput { MaxResultCount = dataManagerRequest!.Take, SkipCount = dataManagerRequest!.Skip }
                : JsonSerializer.Deserialize<GetIcdsInput>(filterJson);
            ...
            var result = await icdAppService.GetListAsync(filter!);
